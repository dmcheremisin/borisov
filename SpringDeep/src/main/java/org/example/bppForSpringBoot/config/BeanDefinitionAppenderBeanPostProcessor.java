...
public class BeanDefinitionAppenderBeanPostProcessor implements BeanPostProcessor {
    ...
    /*
    В Spring Boot это не отработает. Так как BeanPostProcessor не имеет права брать BeanDefinition и коректировать их.
    ...
    private boolean neededActionBehavior(Object bean) {
        return Arrays.stream(bean.getClass().getMethods()).anyMatch(method -> method.isAnnotationPresent(Init.class));
    }
